...
class Equinenetwork_Gam_V2_Public {
	...
	public function inject_stacker( $content ) {
		...
		if ( ! is_array( $stackers ) ) {
			$stackers = array();
		}

		// Filter to active stackers only.
		$active = array_filter( $stackers, function( $s ) { return ! empty( $s['active'] ); } );
		if ( empty( $active ) ) {
			return $content;
		}
